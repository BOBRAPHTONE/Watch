...
FATSDDIR  = $(APPLIBDIR)/fat_sd

#headers to watch
INCLUDESDEP = defines.h

# List C source files here. (C dependencies are automatically generated.)
# use file-extension c for "c-only"-files
## Application sources:
SRC = main.c lcd.c utils.c gps.c it_handlers.c 
SRC += u8g/u8g_font.c u8g/u8g_ll_api.c u8g/u8g_clip.c u8g/u8g_font_6x10r.c
SRC += u8g/u8g_font_5x7r.c u8g/u8g_font_gdr30n.c
...
LDFLAGS +=-T$(LINKERSCRIPTPATH)/$(LSCRIPT)
...
OOCD_CL+=-f interface/buspirate.cfg -f target/stm32f1x.cfg
# initialize
OOCD_CL+=-c init
...
oocdserver:
	cd $(OUTDIR) & openocd -f interface/buspirate.cfg -f target/stm32f1x.cfg

flash:$(OUTDIR)/$(TARGET).bin
	$(OUTDIR)/flash.pl $(OUTDIR)/$(TARGET).bin
...
.PRECIOUS : $(ALLOBJ)
%.elf:  $(ALLOBJ) $(LINKERSCRIPTPATH)/$(LSCRIPT)
	@echo $(MSG_LINKING) $@
...
$(OUTDIR)/$(notdir $(basename $(1))).o : $(1) ${INCLUDESDEP}
